...
def quick_sort(nums, start, end):
    ...
    while left <= right:  # until left is smaller than the right
        ...
        if left > right:  # if left and right is crossed
            # switch smallest data and pivot, so numbers before pivot is all smaller than pivot
            nums[right], nums[pivot] = nums[pivot], nums[right]
        else:  # not crossed
            # switch left and right because left is smaller than pivot and right is bigger than pivot
